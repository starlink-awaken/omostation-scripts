...
# 2. kairon 单元测试
echo "2. kairon 单元测试..."
if [ -d "$REPO_ROOT/projects/kairon/packages" ]; then
    cd "$REPO_ROOT/projects/kairon"
...
# 3. omo 单元测试
echo "3. omo 单元测试..."
if [ -d "$REPO_ROOT/projects/omo/tests" ]; then
    cd "$REPO_ROOT/projects/omo"
...
if ! python3 "$REPO_ROOT/scripts/omo/omo_worker.py" task validate --all-planned >/tmp/omo-planned-validate.log 2>&1; then
...
DONE_BAD=$(
    find "$REPO_ROOT/.omo/tasks/done" -mindepth 2 -maxdepth 2 -name "*.yaml" -print0 2>/dev/null \
        | xargs -0 grep -El "status: (pending|review|in_progress|planned)" 2>/dev/null || true
)
REGISTRY_DONE_BAD=$(
    find "$REPO_ROOT/.omo/tasks/registry/done" -maxdepth 1 -name "*.yaml" -print0 2>/dev/null \
        | xargs -0 grep -El "status: (pending|review|in_progress|planned)" 2>/dev/null || true
)
