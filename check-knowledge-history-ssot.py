...
TARGETS = {
    ...
    ".omo/_knowledge/management/architecture-final-state-v1.md": (
        "历史最终态设计提案 / reference only",
        "不是当前项目布局、当前代码规模、当前层间归属或当前执行真相 SSOT",
        "/.omo/PROJECTS.yaml",
        "/.omo/standards/eCOS-v6-Architecture-Alignment.md",
    ),
    ".omo/_knowledge/management/architecture-final-state-v2.md": (
        "历史最终态修订提案 / reference only",
        "不是当前项目数量、当前包边界、当前依赖真相或当前实施状态 SSOT",
        "/.omo/PROJECTS.yaml",
        "/.omo/standards/eCOS-v6-Architecture-Alignment.md",
    ),
    ".omo/_knowledge/management/code-quality-baseline-phase16.md": (
        "历史代码质量基线 / reference only",
        "不是当前包数量、当前 lint 状态、当前测试分布或当前硬编码风险 SSOT",
        "/.omo/PROJECTS.yaml",
    ),
    ".omo/_knowledge/management/dependency-audit-post-phase16.md": (
        "历史依赖审计记录 / reference only",
        "不是当前依赖拓扑、当前消费者数量或当前运行验证状态 SSOT",
        "/.omo/PROJECTS.yaml",
        "/docs/PANORAMA.md",
    ),
    ".omo/_knowledge/management/ARCH-AUDIT-2026-05.md": (
        "历史架构审计记录 / reference only",
        "不是当前项目清单、当前测试状态、当前 Git 健康或当前架构结论 SSOT",
        "/.omo/PROJECTS.yaml",
        "/docs/PANORAMA.md",
    ),
    ".omo/_knowledge/management/append-only-log-schemas-2026-06-09.md": (
        "历史 schema 治理记录 / reference only",
        "不是当前运行频率、当前治理健康分或当前所有 consumer 实装真相 SSOT",
    ),
    ".omo/_knowledge/management/cross-repo-probe-runtime-metaos-2026-06-10.md": (
        "历史跨仓探查记录 / reference only",
        "不是当前 runtime/metaos 能力面、当前 MCP/CLI 数量或当前落地状态 SSOT",
    ),
    ".omo/_knowledge/management/governance-debt-lifecycle-2026-06-10.md": (
    ...
    ".omo/_knowledge/management/phase1-6-comprehensive-review.md": (
        "历史阶段综合评审 / reference only",
        "不是当前架构拓扑、当前工具数量、当前测试状态或当前阶段门禁 SSOT",
        "/.omo/PROJECTS.yaml",
        "/.omo/state/system.yaml",
    ),
    ".omo/_knowledge/management/phase8-analysis-verification.md": (
        "历史阶段分析与验证 / reference only",
        "不是当前任务状态、当前健康分或当前治理结论 SSOT",
        "/.omo/state/system.yaml",
        "/.omo/goals/current.yaml",
    ),
    ".omo/_knowledge/management/phase9-debt-cleanup-plan.md": (
        "历史债务规划输入 / reference only",
        "不是当前债务账本、当前阶段状态或当前健康分 SSOT",
        "/.omo/debt/registry.yaml",
        "/.omo/state/system.yaml",
    ),
    ".omo/_knowledge/management/phase10-cross-audit.md": (
        "历史交叉审计记录 / reference only",
        "不是当前 system/goals/control 状态或当前审计结论 SSOT",
        "/.omo/state/system.yaml",
        "/.omo/goals/current.yaml",
    ),
    ".omo/_knowledge/management/phase11-cross-audit.md": (
    ...
    ".omo/_knowledge/management/x-plane-architecture-design-v1.md": (
        "历史控制平面设计提案 / reference only",
        "不是当前 X1-X4 实装状态、当前健康分构成或当前控制面执行真相 SSOT",
        "/.omo/standards/omo-governance-surfaces.md",
        "/.omo/state/system.yaml",
    ),
    ".omo/_knowledge/management/opc-p2-t1-memory-boundary-2026-06-11.md": (
        "历史 OPC 设计输入 / reference only",
        "不是当前记忆边界实现状态、当前 owner 分配或当前系统事实 SSOT",
        "/.omo/PROJECTS.yaml",
        "/docs/PANORAMA.md",
    ),
    ".omo/_knowledge/management/opc-p2-t3-recall-flow-2026-06-11.md": (
        "历史 OPC 流程设计输入 / reference only",
        "不是当前 collect/ingest/search/output/archive 落地真相或当前验收结果 SSOT",
        "/.omo/state/system.yaml",
    ),
    ".omo/_knowledge/management/opc-p6-t1-evolution-loop-2026-06-11.md": (
